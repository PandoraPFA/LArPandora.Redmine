...
//______________________________________________
    ...
    for(unsigned int i = 0; i < larListHandle->size(); ++i)
     ...
     for(unsigned int a = 0; a < parIn.size(); ++a){
       if(abs(parIn[a]->PdgCode())==13 && parIn[a]->Process()=="primary"){  // if CCQE numubar event

	for(unsigned int b = 0; b < parIn.size(); ++b){
	  if(abs(parIn[b]->PdgCode())==3122 && parIn[b]->Process()=="primary"){ // if Lambda is in the Primary Particles
	    int LID = parIn[b]->TrackId();

	    for(unsigned int c = 0; c < parIn.size(); ++c){ 
	      if(parIn[c]->Mother()== LID && parIn[c]->Process()=="Decay" && (parIn[c]->PdgCode()==2212)){ // if lambda decays to proton
		//break;
		...
		std::cout << "number of trajectory points of muon = " << traj_points_muon << std::endl;
		...
		std::cout << "number of trajectory points of proton = " << traj_points_proton << std::endl;
		...
		std::cout << "theta of Muon, proton, pion = " << Muon_theta << " " << Proton_theta << " " << Pion_theta << std::endl;
		...
		std::cout << "phi of Muon, proton, pion = " << Muon_phi << " " << Proton_phi << " " << Pion_phi << std::endl;
...
// 					  << " to the current hit";
  ...
  for(unsigned int i=0; i<cluIn.size();++i){
	...
	art::PtrVector<recob::Hit> uclu_hits;
	...
	//while( reco_itr_U != reco_hits_U.end() ){
	  ...
	  std::vector<cheat::TrackIDE> reco_eveides_U   = cheat::BackTracker::HitToEveID(plist, *(scs[(reco_hits_U[j])->Channel()]), reco_hits_U[j]);
...
//  					  << " contributed " << (*reco_idesitr_U).energyFrac; 
	...
	std::cout << "total, mu , p and pi hits are "<< total_number_of_hits_U << " " << mu_hits_U << " "<< p_hits_U<< " " << pi_hits_U << std::endl;
	...
	art::PtrVector<recob::Hit> vclu_hits;
	...
	//while( reco_itr_U != reco_hits_U.end() ){
	  ...
	  std::vector<cheat::TrackIDE> reco_eveides_V   = cheat::BackTracker::HitToEveID(plist, *(scs[(reco_hits_V[j])->Channel()]), reco_hits_V[j]);
