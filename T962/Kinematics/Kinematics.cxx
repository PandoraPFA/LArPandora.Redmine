...
namespace kin {
//-----------------------------------------------------------------------------
  Kinematics::Kinematics(fhicl::ParameterSet const& pset) :
  ...
  void Kinematics::analyze(const art::Event& evt) 
  {
    ...
    art::FindManyP<recob::SpacePoint> fmsp(LarTrackHandle, evt, fLarTracks_label);
    
    for(size_t i = 0; i < LarTrackHandle->size(); ++i){
      art::Ptr<recob::Track> lartrack(LarTrackHandle,i);
      
      // get the space points associated with the track
      std::vector< art::Ptr<recob::SpacePoint> > sps = fmsp.at(i);
