...
namespace larg4 {
  ...
  OpBoundaryProcessSimple::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
  {
	...
	if (aMaterialPropertiesTable) {
	  ...
	  if (Reflectance) 
	    {
	      double theReflectance = Reflectance->GetProperty(thePhotonMomentum);
	      if( G4BooleanRand(theReflectance))
		{
		  if(DiffuseReflectanceFraction)
		    {
		      double theDiffuseReflectanceFraction = DiffuseReflectanceFraction->GetProperty(thePhotonMomentum);
