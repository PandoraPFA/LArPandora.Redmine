...
namespace larg4 {
  ...
  void ModularPhysicsList::ConstructProcess()
  {
    ...
    G4ParallelWorldScoringProcess* PMTParallelWorldScoringProcess
      = new G4ParallelWorldScoringProcess("PMTReadoutScoringProcess");

    // Note that the name below MUST match the name used in the
    // LArVoxelReadoutGeometry or PMTReadoutGeometry constructor.
    LArVoxelParallelWorldScoringProcess->SetParallelWorld("LArVoxelReadoutGeometry");
    PMTParallelWorldScoringProcess->SetParallelWorld("PMTReadoutGeometry");

    // Tell all the particles in the physics list to recognize the
    // LAr voxel parallel world.  "theParticleIterator" is defined in
    // G4VUserPhysicsList.hh.  Only photons recognise the PMT parallel
    // world.
	...
	pmanager->AddProcess(PMTParallelWorldScoringProcess);
	pmanager->SetProcessOrderingToLast(PMTParallelWorldScoringProcess, idxAtRest);
	pmanager->SetProcessOrdering(PMTParallelWorldScoringProcess, idxAlongStep, 1);
	pmanager->SetProcessOrderingToLast(PMTParallelWorldScoringProcess, idxPostStep);
      }
	...
	G4bool cE(false);
	if ((
	     (particle->GetParticleName() == G4MuonPlus::MuonPlus()->GetParticleName() || 
	...
	if (genSecondaries){
	  ...
	  mf::LogInfo("PhysicsList: ") << "Turning on WrappedMuNuclear for " 
				       ...
				       << "s with " << nSecondaries 
				       << " appropriately weighted secondaries. XSBias is set to " 
				       << xSBias << " and the cross-section is increased by a factor of " 
				       << fXSBias << ".";
	  G4MuNuclearInteraction* g4MNI = new G4MuNuclearInteraction();
	...
	if (cE){
	  ...
	  //p->AddDataSet(new G4UElasticCrossSection(particle));
	  p->RegisterMe(model);
