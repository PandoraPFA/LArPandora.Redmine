...
///      sim::ParticleList* particleList = // ...;
///      for ( sim::ParticleList::const_iterator i = particleList->begin();
...
namespace sim {
  ...
  public:
    ...
    // above.
    void Add( const ParticleList& );
    ParticleList& operator+=( const ParticleList& other ) 
    { 
      this->Add(other); 
      return *this; 
    }
    const ParticleList operator+(const ParticleList& other) const 
    {
      return ParticleList(*this) += other;
    }
    ...
    ParticleList Add(const int& offset) const;
    ParticleList operator+(const int& value) const 
    {
      return Add(value);
    }
    ...
    friend ParticleList operator+(const int& value, const ParticleList& list);

    // Standard STL methods, to make this class look like an STL map.
    // Again, if you don't know STL, you can just ignore these
    // methods.
    iterator               begin()        { return m_particleList.begin(); }
    const_iterator         begin()  const { return m_particleList.begin(); }
    iterator               end()          { return m_particleList.end(); }
    const_iterator         end()    const { return m_particleList.end(); }
    reverse_iterator       rbegin()       { return m_particleList.rbegin(); }
    const_reverse_iterator rbegin() const { return m_particleList.rbegin(); }
    reverse_iterator       rend()         { return m_particleList.rend(); }
    const_reverse_iterator rend()   const { return m_particleList.rend(); }

    size_type size()           const { return m_particleList.size(); }
    bool empty()               const { return m_particleList.empty(); }
    void swap( ParticleList& other ) { m_particleList.swap( other.m_particleList ); }

    iterator       find(const key_type& key)              { return m_particleList.find(key); }
    const_iterator find(const key_type& key)        const { return m_particleList.find(key); }
    iterator       upper_bound(const key_type& key)       { return m_particleList.upper_bound(key); }
    const_iterator upper_bound(const key_type& key) const { return m_particleList.upper_bound(key); }
    iterator       lower_bound(const key_type& key)       { return m_particleList.lower_bound(key); }
    const_iterator lower_bound(const key_type& key) const { return m_particleList.lower_bound(key); }
  ...
  private:
    ...
    primaries_type                         m_primaries; ///< Sorted list of the track IDs of primary particles.
