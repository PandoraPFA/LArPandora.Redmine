...
namespace sim {
  ...
  PhotonVoxel::PhotonVoxel(Double_t xMin, Double_t xMax, Double_t yMin, Double_t yMax, Double_t zMin, Double_t zMax, int N)
  {
    xVoxelMin = xMin;
    xVoxelMax = xMax;
    yVoxelMin = yMin;
  ...
  PhotonVoxelDef::PhotonVoxelDef(Double_t xMin, Double_t xMax, int xN, Double_t yMin, Double_t yMax, int yN, Double_t zMin, Double_t zMax, int zN)
  {
    fxSteps = xN;
    fySteps = yN;
    fzSteps = zN;
  ...
  int PhotonVoxelDef::GetVoxelID(TVector3 Position) const
  {
	...
	std::cout<<"PhotonVoxelDef warning : Looked up voxel outside of volume" <<std::endl;
        ID = -1;
      }
  ...
  PhotonVoxel PhotonVoxelDef::GetPhotonVoxel(int ID) const
  {
    ...
    Double_t xMin = VoxelSize[0] * (xStep)   + fLowerCorner[0];
    Double_t xMax = VoxelSize[0] * (xStep+1) + fLowerCorner[0];
    Double_t yMin = VoxelSize[1] * (yStep)   + fLowerCorner[1];
    Double_t yMax = VoxelSize[1] * (yStep+1) + fLowerCorner[1];
    Double_t zMin = VoxelSize[2] * (zStep)   + fLowerCorner[2];
    Double_t zMax = VoxelSize[2] * (zStep+1) + fLowerCorner[2];

    std::cout<<"Photon Voxel Def Debug : " << xStep<<" " <<yStep<<" " <<zStep<<std::endl
