...
namespace sim{
  ...
  sim::LArVoxelList SimListUtils::GetLArVoxelList(const art::Event& evt, std::string moduleLabel)
  {
    ...
    for(std::vector<const sim::SimChannel*>::const_iterator itr = sccol.begin(); itr != sccol.end(); ++itr){
  ...
  //----------------------------------------------------------------------
  // moduleLabel is the label of the module that created the particles you 
  // are putting into the list
  sim::ParticleList SimListUtils::GetParticleList(const art::Event& evt, std::string moduleLabel)
  {
    /// get the particles from the event handle
    art::Handle< std::vector<sim::Particle> > parHandle;
    evt.getByLabel(moduleLabel, parHandle);

    sim::ParticleList list;

    for(unsigned int i = 0; i < parHandle->size(); ++i){
      art::Ptr<sim::Particle> p(parHandle, i);      
      sim::Particle *part = new sim::Particle(*p);
      list.Add(part);
    }
  ...
  // are putting into the list
  sim::PMTHitCollection SimListUtils::GetPMTHitCollection(const art::Event& evt, std::string moduleLabel)
  {
    /// get the voxels from the event handle
    art::Handle< std::vector<sim::PMTHit> > pmtHandle;
    evt.getByLabel(moduleLabel, pmtHandle);
    const std::vector<sim::PMTHit>& pmt(*pmtHandle);

    sim::PMTHitCollection pmtList;

    /// loop over the pmthits and put them into the list
    for(std::vector<sim::PMTHit>::const_iterator itr = pmt.begin(); itr != pmt.end(); ++itr){
      /// make an entry in the list for this pmt id
      pmtList.GetHit((*itr).PMTID())->SetID((*itr).PMTID());

      /// add the photons to the entry
      for(sim::PMTHit::const_iterator pitr = (*itr).begin(); pitr != (*itr).end(); ++pitr)
	pmtList.GetHit((*itr).PMTID())->push_back((*pitr));
    }
