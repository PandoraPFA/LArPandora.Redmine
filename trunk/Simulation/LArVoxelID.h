...
///    voxelID.Print();                    // Standard ROOT method
...
namespace sim {
  ...
  public:
    ...
    int XBin() const { return fbins[0]; }
    int YBin() const { return fbins[1]; }
    int ZBin() const { return fbins[2]; }
    int TBin() const { return fbins[3]; }
    ...
    // How Print() is implemented, and also handy for debugging: Put
    // the contents on the output stream.
