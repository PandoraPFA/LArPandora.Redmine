...
namespace sim {
  ...
  //-------------------------------------------------
  const sim::Particle* ParticleList::Primary( const int index ) const
  {
  ...
  //-------------------------------------------------
  sim::Particle* ParticleList::Primary( const int index )
  {
    // Advance "index" entries from the beginning of the primary list.
    primaries_const_iterator primary = m_primaries.begin();
    std::advance( primary, index );
...
// 						   particle->PrimaryIndex(),
  ...
  void ParticleList::insert( sim::Particle* particle ) 
  { 
    ...
    // If this is a primary particle, add it to the list.
    if ( !particle->Primary().isNull() )
  ...
  {
    iterator entry = m_particleList.find( key );
    delete (*entry).second;
    return m_particleList.erase( key );
  }
  ...
  {
    const_iterator i = m_particleList.find(key);
    assert(i != m_particleList.end()); 
    return (*i).second;
  }
  ...
  // Static variable for eve ID calculator. We're using an auto_ptr,
  // so when this object is eventually deleted (at the end of the job)
  // it will delete the underlying pointer.
  static std::auto_ptr<EveIdCalculator> eveIdCalculator;
