...
namespace sim {
  ...
  {
    // If there's nothing but unassigned energy for this voxel, just
    // print that.
	...
	output << unassigned;
      ...
      {
	// Otherwise, display the total energy then the breakdown of
	// the sum.
	output << data.Energy() << " = <ID,E>=";
	for ( LArVoxelData::const_iterator begin = data.begin(), i = begin, end = data.end();
	      i != end; ++i )
	  {
	    if ( i != begin )
	      output << ",";
