...
///
/// \version $Id: LArVoxelData.h,v 1.4 2010/03/03 20:40:24 t962cvs Exp $
...
namespace sim {
  ...
  public:
    ...
    mapped_type UnassignedEnergy() const { return fenergy; }
    ...
    size_type NumberParticles() const { return size(); }
    ...
    // exist, create it.
    void Add( const mapped_type& energy, const key_type& trackID ) { ftrackEnergy[trackID] += energy; }

    // If there's no key, it must be "unassigned" energy.
    void Add( const mapped_type& energy ) { fenergy += energy; }

    sim::LArVoxelID VoxelID()                         const {return fVoxelID;   }
    void            SetVoxelID(sim::LArVoxelID voxID)       { fVoxelID = voxID; }
    ...
    iterator               begin()        { return ftrackEnergy.begin();  }
    const_iterator         begin()  const { return ftrackEnergy.begin();  }
    iterator               end()          { return ftrackEnergy.end();    }
    const_iterator         end()    const { return ftrackEnergy.end();    }
    reverse_iterator       rbegin()       { return ftrackEnergy.rbegin(); }
    const_reverse_iterator rbegin() const { return ftrackEnergy.rbegin(); }
    reverse_iterator       rend()         { return ftrackEnergy.rend();   }
    const_reverse_iterator rend()   const { return ftrackEnergy.rend();   }

    size_type size()                const { return ftrackEnergy.size();   }
    bool      empty()               const { return ftrackEnergy.empty();  }
    ...
    void clear() { fenergy = 0.; ftrackEnergy.clear(); }

    iterator       find(const key_type& key)              { return ftrackEnergy.find(key);        }
    const_iterator find(const key_type& key)        const { return ftrackEnergy.find(key);        }
    iterator       upper_bound(const key_type& key)       { return ftrackEnergy.upper_bound(key); }
    const_iterator upper_bound(const key_type& key) const { return ftrackEnergy.upper_bound(key); }
    iterator       lower_bound(const key_type& key)       { return ftrackEnergy.lower_bound(key); }
    const_iterator lower_bound(const key_type& key) const { return ftrackEnergy.lower_bound(key); }
    size_type      erase( const key_type& key )           { return this->RemoveTrack(key);         }

    mapped_type&       operator[](const key_type& key)        { return ftrackEnergy[key];    }
    // My own little addition: operator[] in a const context.
    const mapped_type& operator[]( const key_type& key) const { return ftrackEnergy.at(key); }
    mapped_type&       at(const key_type& key)                { return ftrackEnergy.at(key); }
    const mapped_type& at(const key_type& key)          const { return ftrackEnergy.at(key); }

    // In addition to operator[], include one insert() method.  Note
    // that, as with operator[], there's no check against overwriting
    // an existing item.
    void insert( const key_type& key, const mapped_type& value ) { ftrackEnergy[key] = value; }
