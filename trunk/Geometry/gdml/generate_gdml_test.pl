...
EOF
	...
	print GDML <<EOF;
	    <physvol>
	     ...
	     <position name="posTPCWireVert$i" unit="cm" z="(-0.5*kTPCLength)+(($i+1)*kTPCWirePitch)" x="0" y="0"/>
...
EOF
  ...
  for ($i = 0; $i < $NumberSideWires; ++$i)
  {
    ...
    <physvol>
     ...
     <position name="posTPCWire$i" unit="cm" y="-0.5*kTPCWidth+0.5*($i+1)*kTPCWireXPitch/kTanUVAngle" z="0.5*(-kTPCLength+($i+1)*kTPCWireXPitch)" x="0"/>
...
sub gen_tpcplane_bad()
{
    ...
    my $NumberSideWires = int( $TanUVAngle * $TPCWidth / $TPCYWirePitch ) - 1;
