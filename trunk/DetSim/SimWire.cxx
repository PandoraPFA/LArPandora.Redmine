...
namespace detsim{
  ...
  //-------------------------------------------------
  void SimWire::GenNoise(std::vector<double>& noise)
  {
    art::ServiceHandle<art::RandomNumberGenerator> rng;
    CLHEP::HepRandomEngine &engine = rng->getEngine();
    CLHEP::RandFlat flat(engine);
  ...
  void SimWire::SetElectResponse()
  {
    ...
    if( geo->DetId() == geo::kArgoNeuT ){
      ...
      if(inFile.IsZombie()){      
	std::cerr << "Cannot open response file" << fResponseFile.c_str() << std::endl;      
	// 	assert(0);    
      }    
	...
	fElectResponse[i] = 120000.0*exp(-time[i]/fShapeTimeConst[0])/(1. + exp(-time[i]/fShapeTimeConst[1]))/norm;  // The 120000 is an arbitrary scaling to get displays for microboone
    ...
    for(unsigned int i = 0; i < fNElectResp; ++i){
      //std::cout <<"checking ElectResponse: i=  "<< i << "  time[i]=  " << time[i] << "  fElectResponse[i]=  " << fElectResponse[i] << std::endl;
